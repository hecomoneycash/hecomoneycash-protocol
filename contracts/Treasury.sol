 ...
 * @notice Monetary policy logic to adjust supplies of hecomoney cash assets
...
contract Treasury is ContractGuard, Epoch {
    ...
    uint256 public maxPercentage = 30;
        ...
        checkOperator
    {
        ...
        percentage = Math.min(percentage, maxPercentage * 1e18);
    ...
    function setMaxPercentage(uint256 _maxPercentage)
        external
        onlyOwner
    {
    ...
    function setStartTime_(uint256 _starttime) external onlyOwner{
        startTime = _starttime;
        lastExecutedAt = _starttime;
    }

    function setPeriod_(uint256 _period) external onlyOwner{
        period = _period;
    }
